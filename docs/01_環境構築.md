...
```powershell
mkdir "$HOME\Documents\git-projects" -Force   # 置き場フォルダを作成（-Force は既存でもエラーにしない）
...
```powershell
git clone <リポジトリのURL>     # リポジトリ一式をダウンロード（URLは前任者に確認）
```
...
BASE_DIR = Path(__file__).resolve().parent.parent  # プロジェクトのルートフォルダの絶対パスを計算
...
DEBUG=env.bool('DEBUG')                       # .envの DEBUG を真偽値(True/False)として取り出す
```
...
```python
ALLOWED_HOSTS = [
    'hozen-kosu-react-cwaqashkafgbg5e5.japaneast-01.azurewebsites.net',  # 本番のドメイン
    'localhost',                                                          # 自分のPC（開発用）
    '127.0.0.1'                                                           # 自分のPCのIPアドレス表記
    ]
```
...
```python
REST_FRAMEWORK = {
    'DEFAULT_PAGINATION_CLASS': 'rest_framework.pagination.PageNumberPagination',  # ページ番号方式の分割
...
DATABASES = {
    'default': {                                       # 既定のデータベース接続
        'ENGINE': 'django.db.backends.postgresql',     # PostgreSQLを使う指定（§6で入れたもの）
        'NAME': os.getenv('DB_NAME', ''),              # データベース名を環境変数DB_NAMEから取得
        'USER': os.getenv('DB_USER', ''),              # ユーザー名を環境変数DB_USERから取得
        'PASSWORD': os.getenv('DB_PASSWORD', ''),      # パスワードを環境変数DB_PASSWORDから取得
        'HOST': os.getenv('DB_HOST', ''),              # 接続先ホストを環境変数DB_HOSTから取得
        'PORT': '5432',                                # ポート番号は5432固定（PostgreSQLの既定）
    }
}
```
1行ずつ、`.env` との対応：
...
STATICFILES_STORAGE = 'whitenoise.storage.CompressedManifestStaticFilesStorage'  # 静的ファイルを圧縮配信
```
