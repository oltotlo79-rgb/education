...
```bash
npm install                       # package.json を読んで全部入れる（フロントの frontend/ で実行）
```
...
```python
MIDDLEWARE = [
    'corsheaders.middleware.CorsMiddleware',                         # ← cors-headers：別オリジン通信を許可（一番先頭が重要）
    'django.middleware.security.SecurityMiddleware',                 # Django標準：セキュリティ
    'django.contrib.sessions.middleware.SessionMiddleware',         # Django標準：セッション
    ...
    'django.middleware.clickjacking.XFrameOptionsMiddleware',       # Django標準：クリックジャッキング対策
    'whitenoise.middleware.WhiteNoiseMiddleware',                   # ← WhiteNoise：静的ファイル配信
    'kosu.middleware.clear_session_middleware.CurrentRequestMiddleware',  # ← 自作ミドルウェア
]
```
...
```python
REST_FRAMEWORK = {
    'DEFAULT_PAGINATION_CLASS': 'rest_framework.pagination.PageNumberPagination',  # 一覧をページ分割する方式
    'PAGE_SIZE': 20,                                                              # 1ページ20件
    'DEFAULT_PARSER_CLASSES': (
        'rest_framework.parsers.JSONParser',                                     # 受信はJSONだけ受け付ける
    ),
    'DEFAULT_RENDERER_CLASSES': (
        'rest_framework.renderers.JSONRenderer',                                 # 送信はJSONで返す
    ),
}
```
...
```python
WSGI_APPLICATION = 'hozen_another.wsgi.application'   # Gunicornはこの application を呼び出してDjangoを起動する
```
...
```tsx
import React from 'react';                                                          // React本体（画面の組み立て役）
import ReactDOM from 'react-dom/client';                                            // ReactDOM（画面をブラウザに描く役）
import { BrowserRouter as Router, Routes, Route, useLocation } from "react-router-dom";  // ルーター（URLと画面の対応付け）
import './styles/global.css';                                                       // 共通CSS（全画面の見た目の土台）
import Login from './MainPage/Login';                                               // ↓ここから下は各画面の部品（自作）
...
const App: React.FC = () => {                       // App という画面全体の親部品を定義
  ...
  React.useEffect(() => {                            // 「URLが変わるたびに」実行する処理を登録
    const routeTitles: { [key: string]: string } = {  // URL → タイトル文字列 の対応表（オブジェクト）
      "/login": "ログイン - 業務工数システム",          // /login を開いたらこのタイトル
      "/help": "ヘルプ - 業務工数システム",
      "/": "Main Menu - 業務工数システム",
      // …（中略：全URLぶんの対応）…
    ...
    document.title = routeTitles[location.pathname] || "業務工数システム";  // 対応表にあればそのタイトル、なければ既定値
  }, [location]);                                     // [location] = 「locationが変わったら再実行」の指示
```
...
```tsx
const root = ReactDOM.createRoot(document.getElementById('root') as HTMLElement);  // HTMLの<div id="root">を画面の土台にする

root.render(                                  // その土台に描く
  <React.StrictMode>                          {/* 開発中に潜在バグを警告してくれる「厳格モード」で包む */}
...
1. 1つだけアンインストール（`frontend/` で実行）。
   ```bash
   npm uninstall @fullcalendar/daygrid   # 1部品だけ取り除く（package.jsonからも自動で消える）
   ```
2. ビルドが通るか確認。
   ```bash
...
4. 開発サーバーで画面を実際に触る。
   ```bash
   npm start                              # localhost:3000 で各画面（特にカレンダー・グラフ）を確認
   ```
