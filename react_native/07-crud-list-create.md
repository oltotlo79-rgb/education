...
`useState` に `<Book[]>`（山カッコで型を指定）を付けて「このstateはBookの配列だ」と明示しています。これで `books` に対して `item.title` などを書くとき、TypeScriptが補完・チェックしてくれます。
...
> **`useCallback` とは？** `useCallback`（ユーズ・コールバック）は「関数を毎回作り直さず、覚えておく（メモする）」フックです。`useFocusEffect` と組み合わせるときの定番の書き方なので、ここでは「おまじない」として `useFocusEffect(useCallback(() => { load() }, [load]))` の形を覚えておけば十分です。
